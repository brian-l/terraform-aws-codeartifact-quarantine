...
- **Proactive cache-fill** for staging (opt-in). A new scheduled Lambda (`<name>-proactive-fill`, default `rate(1 hour)`) populates staging with upstream versions *before* developers request them, so the scan + cooldown + audit chain runs ahead of consumer demand. Two stacked modes: **follow-mode** (always on when enabled — watches every package already cached in staging and fetches new upstream versions) and **allowlist-mode** (`var.pipeline.proactive_fill.allowlist = [{ format, name, include_prereleases? }]` — opt-in entries pulled even if not yet in staging). Pre-releases skipped by default; per-run cap via `max_fetches_per_run` (default 200). Fetch mechanism is an authenticated HTTPS GET against the staging endpoint — same as `npm install` / `pip install` — so the existing EventBridge → ingestion → pipeline path handles the rest. Audit rows land in the existing promotion table with `record_type = "proactive_fill"`. New output `proactive_fill_lambda_arn`. Set `var.pipeline.proactive_fill.enabled = true` to opt in; default is `false`, so upgrades from 0.1.1 don't create resources without consent.
- **`record_type` discriminator on audit rows.** `common.audit.record()` now writes a `record_type` field defaulting to `"promotion"`. Proactive-fill writes use `"proactive_fill"`. Reserved for future event classes (yank-detected fills, etc.). **Note for consumers querying the audit table**: rows written by pre-0.1.2 versions of the module have no `record_type` field — treat absence as `"promotion"` in any FilterExpression.
- **Yank / unpublish / malware-advisory detection** for cached package versions (opt-in). A scheduled Lambda (default `rate(1 hour)`) enumerates every version in the staging repos and checks each against PyPI yank metadata (PEP 592), npm packument deprecations + missing-from-packument unpublish detection, and OSV.dev malware advisories. On a newly observed yank/unpublish/malicious verdict the Lambda always publishes to the existing SNS topic + writes to a sibling `<name>-yank-audit` DynamoDB table, and (per the configured response per status) calls `UpdatePackageVersionsStatus` / `DisposePackageVersions` / `DeletePackageVersions` on both the staging and prod copies. Configurable via the new `var.pipeline.yank_detection` block: `enabled` (default `false` — set to `true` to opt in), `schedule`, `sources` (subset of `upstream` / `osv`), and `response.yanked` / `response.unpublished` / `response.malicious` (one of `alert` / `unlist` / `dispose` / `delete`). Defaults when enabled: yanked→unlist, unpublished→dispose, malicious→dispose. State preserved across runs in `<name>-yank-state` for idempotency. New outputs `yank_audit_table_name`, `yank_audit_table_arn`, `yank_check_lambda_arn`. See README → "Handling yanked / unpublished / malicious packages" for the operational notes; SECURITY.md flags the cached-canonical caveat that motivates enabling this for any real deployment.
