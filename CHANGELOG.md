...
- **Proactive cache-fill** for staging. A new scheduled Lambda (`<name>-proactive-fill`, default `rate(1 hour)`) populates staging with upstream versions *before* developers request them, so the scan + cooldown + audit chain runs ahead of consumer demand. Two stacked modes: **follow-mode** (always on when enabled — watches every package already cached in staging and fetches new upstream versions) and **allowlist-mode** (`var.pipeline.proactive_fill.allowlist = [{ format, name, include_prereleases? }]` — opt-in entries pulled even if not yet in staging). Pre-releases skipped by default; per-run cap via `max_fetches_per_run` (default 200). Fetch mechanism is an authenticated HTTPS GET against the staging endpoint — same as `npm install` / `pip install` — so the existing EventBridge → ingestion → pipeline path handles the rest. Audit rows land in the existing promotion table with `record_type = "proactive_fill"`. New output `proactive_fill_lambda_arn`. Default `enabled = true`.
- **`record_type` discriminator on audit rows.** `common.audit.record()` now writes a `record_type` field defaulting to `"promotion"`. Proactive-fill writes use `"proactive_fill"`. Reserved for future event classes (yank-detected fills, etc.). **Note for consumers querying the audit table**: rows written by pre-`[Unreleased]` versions of the module have no `record_type` field — treat absence as `"promotion"` in any FilterExpression.
